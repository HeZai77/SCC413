...
from arch.ops import conv_norm_relu, dconv_norm_relu, ResidualBlock, get_norm_layer, init_network
from arch.transformerConv import Transformer
...
class ResnetTransGenerator(nn.Module):
    def __init__(self, input_nc=3, output_nc=3, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=True, num_blocks=6):
        super(ResnetTransGenerator, self).__init__()
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d

        res_encoder = [ #nn.ReflectionPad2d(3),
                        conv_norm_relu(input_nc, ngf * 1, 3, norm_layer=norm_layer, bias=use_bias),
                        conv_norm_relu(ngf * 1, ngf * 2, 3, 2, 1, norm_layer=norm_layer, bias=use_bias),
                        conv_norm_relu(ngf * 2, ngf * 4, 3, 2, 1, norm_layer=norm_layer, bias=use_bias)]

        for i in range(num_blocks):
            res_encoder += [ResidualBlock(ngf * 4, norm_layer, use_dropout, use_bias)]

        dim = ngf * 4
        self.transformer = Transformer(dim=dim, proj_kernel=3, kv_proj_stride=2, depth=3, heads=6, mlp_mult=4, dropout=0.05)

        res_decoder = [dconv_norm_relu(ngf * 4, ngf * 2, 3, 2, 1, 1, norm_layer=norm_layer, bias=use_bias),
                      dconv_norm_relu(ngf * 2, ngf * 1, 3, 2, 1, 1, norm_layer=norm_layer, bias=use_bias),
                      nn.ReflectionPad2d(1),
                      nn.Conv2d(ngf, output_nc, kernel_size=3),
                      nn.Tanh()]

        self.res_encoder = nn.Sequential(*res_encoder)
        self.res_decoder = nn.Sequential(*res_decoder)
...
def define_Gen(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, gpu_ids=[0]):
    ...
    elif netG == 'resnetTrans_9blocks':
        gen_net = ResnetTransGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, num_blocks=9)
    elif netG == 'unetTrans':
        from arch.generators_v2 import Generator
        gen_net = Generator(conv_dim=64, norm_fun='InstanceNorm', act_fun='ReLU', use_sn=True)
    else:
...
if __name__ == '__main__':
    ...
    Gen = define_Gen(input_nc=1, output_nc=1, ngf=64, netG="resnetTrans_9blocks", norm="instance", use_dropout=True, gpu_ids=[])
