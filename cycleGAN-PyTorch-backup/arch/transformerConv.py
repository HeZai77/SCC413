...
class ConvTransformer(nn.Module):
    def __init__(self, input_nc, n_downsampling, depth, heads, proj_kernel=3, mlp_mult=4, dropout=0., ngf=16):
        super().__init__()
        ...
        self.transformer = Transformer(dim=dim, proj_kernel=proj_kernel, kv_proj_stride=2, depth=depth, heads=heads, mlp_mult=mlp_mult, dropout=dropout)
...
if __name__ == '__main__':
    ...
    ConvNet = ConvTransformer(input_nc=1, n_downsampling=3, depth=3, heads=6, dropout=0.05, ngf=64).to(device)

    input = torch.randn(2, 1, 128, 128)
    output = ConvNet(input)
